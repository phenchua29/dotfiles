...
All chezmoi commits use conventional commits with scope: `type(scope): message`.

```
feat(goose): add security config and adversary mode
fix(shell): correct zsh path
...
chezmoi git -- commit -m "chore(dotfiles): resolve merge conflict"
